"""
KNN model training script for music genre classification.
...
def main():
    ...
    setup_logging(
        log_file=f"{config['paths']['logs']}/train_knn.log",
    ...
    model_path = f"{config['paths']['models']}/knn.pkl"
    scaler_path = f"{config['paths']['models']}/knn_scaler.pkl"
    results_path = f"{config['paths']['outputs']}/knn_results.json"
    cm_plot_path = f"{config['paths']['plots']}/knn_confusion_matrix.png"
